...
class ChatExporter:
    ...
    def export_conversation(
        ...
        if format == "markdown":
            ...
            raise ValueError(
                f"Unsupported chat export format {format!r}. " f"Supported: markdown, html, json"
            )
    ...
    def to_html(self, conversation: Conversation) -> str:
        ...
        for msg in conversation:
            ...
            timestamp_html = (
                f'<span class="timestamp">{html_lib.escape(timestamp)}</span>' if timestamp else ""
            )
    ...
    def _render_html_content(content: str) -> str:
        ...
        escaped_parts: List[str] = [
            html_lib.escape(parts[0]) if i == 0 else parts[0] for i, parts in enumerate([parts])
        ]
