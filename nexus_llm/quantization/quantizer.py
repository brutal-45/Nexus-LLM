...
class Quantizer:
    ...
    def get_model_size(self, model: Any) -> int:
        ...
        try:
            ...
            sd = model.state_dict()
            total = sum(
                v.nelement() * v.element_size() for v in sd.values() if isinstance(v, torch.Tensor)
            )
            if total > 0:
                return total
        except Exception:
    ...
    def _quantize_int4(model: Any, config: QuantConfig) -> Any:
        ...
        try:
            ...
            _replace_linear(model)
            logger.debug(
                "Applied BitsAndBytes int4 quantization (group_size=%d)", config.group_size
            )
            return model
        except ImportError:
        ...
        logger.debug(
            "Applied simulated int4 quantization (group_size=%d, sym=%s)",
            config.group_size,
            config.sym,
        )
        return model
