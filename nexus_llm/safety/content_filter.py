...
_SELF_HARM_PATTERNS: List[re.Pattern] = [
    re.compile(r"\b(how\s+to\s+(kill|harm)\s+myself|want\s+to\s+die|end\s+my\s+life)\b", re.IGNORECASE),
...
_HARASSMENT_PATTERNS: List[re.Pattern] = [
    re.compile(r"\b(repeatedly\s+(stalk|harass|bother)|bully\s+online|cyberbully)\b", re.IGNORECASE),
