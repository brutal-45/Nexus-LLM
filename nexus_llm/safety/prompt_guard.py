...
_EXTRACTION_PATTERNS: List[re.Pattern] = [
    ...
    re.compile(
        r"\b(reveal|show|display|print|output)\s+(your|the)\s+(system|initial)\s+prompt\b",
        re.IGNORECASE,
    ),
...
class PromptGuard:
    ...
    def validate_prompt(self, prompt: str) -> Tuple[bool, List[ValidationIssue]]:
        ...
        valid = len(issues) == 0
        if not valid:
            logger.warning("Prompt validation failed: %d issue(s) found", len(issues))
        return valid, issues
    ...
    def check_system_prompt(self, system_prompt: str) -> Tuple[bool, List[str]]:
        ...
        for pattern in self._system_warn:
            ...
            if match:
                warnings.append(f"Unsafe directive in system prompt: '{match.group()}'")

        safe = len(warnings) == 0
        if not safe:
            logger.warning("System prompt check: %d warning(s)", len(warnings))
