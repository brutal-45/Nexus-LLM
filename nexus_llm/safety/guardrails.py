...
class SafetyGuardrails:
    ...
    def validate_output(self, text: str) -> GuardrailResult:
        ...
        for pii_type, pattern in pii_patterns.items():
            ...
            if matches:
                result.add_warning(f"Output may contain {pii_type} ({len(matches)} match(es))")
    ...
    def remove_topic_restriction(self, name: str) -> bool:
        ...
        self._topic_restrictions = [r for r in self._topic_restrictions if r.name != name]
    ...
    def list_blocked_patterns(self) -> List[Dict[str, Any]]:
        ...
        return [
            {"name": name, "severity": severity} for name, _, severity in self._blocked_patterns
        ]
