...
class BatchProcessor:
    ...
    def is_ready(self, item_id: str) -> bool:
        ...
        """
        return (
            item_id in self._results
            or self._pending_futures.get(item_id, threading.Event()).is_set()
        )
    ...
    def _process_batch(self, batch: Batch) -> None:
        ...
        try:
            ...
            batch.status = (
                BatchStatus.COMPLETED if batch.failure_count == 0 else BatchStatus.PARTIAL
            )
        ...
        while self._running:
            try:
                with self._batch_condition:
                    ...
                    if self._current_batch and self._current_batch.size > 0 and self._running:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            total_items_processed = sum(
                b.size
                for b in self._completed_batches.values()
