...
from nexus_llm.workers.inference_worker import InferenceResult, InferenceTask, InferenceWorker
from nexus_llm.workers.task_queue import TaskPriority, TaskQueue
from nexus_llm.workers.training_worker import TrainingResult, TrainingTask, TrainingWorker
...
class WorkerPool:
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        """
        inference_info = [self._worker_info[w.worker_id].to_dict() for w in self._inference_workers]
        training_info = [self._worker_info[w.worker_id].to_dict() for w in self._training_workers]
        ...
        return {
            ...
            "inference_workers": {
                ...
                "busy": sum(
                    1 for w in self._inference_workers if self._worker_info[w.worker_id].is_busy
                ),
            ...
            "training_workers": {
                ...
                "busy": sum(
                    1 for w in self._training_workers if self._worker_info[w.worker_id].is_busy
                ),
    ...
    def _select_inference_worker(self, strategy: str = "round_robin") -> Optional[InferenceWorker]:
    ...
    def _select_training_worker(self) -> Optional[TrainingWorker]:
        ...
        # Prefer least busy
        not_busy = [w for w in alive if not self._worker_info[w.worker_id].is_busy]
        if not_busy:
            return not_busy[0]
