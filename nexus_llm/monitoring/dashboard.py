...
class Dashboard:
    ...
    def render(self) -> str:
        ...
        lines.append(
            f"    Memory:  {system['memory']['used_gb']:.1f}/{system['memory']['total_gb']:.1f} GB ({system['memory']['percent']:.1f}%)"
        )
        lines.append(
            f"    Disk:    {system['disk']['used_gb']:.1f}/{system['disk']['total_gb']:.1f} GB ({system['disk']['percent']:.1f}%)"
        )
        if system["gpu"]["available"]:
            lines.append(f"    GPU:     {system['gpu']['name']}")
            lines.append(
                f"    GPU Mem: {system['gpu']['memory_used_mb']:.0f}/{system['gpu']['memory_total_mb']:.0f} MB ({system['gpu']['utilization_percent']:.1f}%)"
            )
        else:
