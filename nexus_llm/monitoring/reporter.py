...
class StatusReporter:
    ...
    def _collect_component_statuses(self) -> List[ComponentStatus]:
        ...
        try:
            ...
            db_status.status = (
                HealthStatus.HEALTHY if health.get("healthy") else HealthStatus.UNHEALTHY
            )
    ...
    def _collect_resource_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            summary["gpu_memory_percent"] = (
                round(mem_info.used / mem_info.total * 100, 1) if mem_info.total else 0
            )
    ...
    def _format_markdown(self, report: StatusReport) -> str:
        ...
        for comp in report.components:
            ...
            lines.append(
                f"| {comp.name} | {comp.status.value} | {comp.message or '-'} | {latency} |"
            )
