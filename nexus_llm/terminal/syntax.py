...
FALLBACK_PATTERNS: dict[str, list[tuple[str, re.Pattern[str]]]] = {
    Language.PYTHON: [
        ...
        (
            "builtin",
            re.compile(
                r"\b(print|len|range|int|str|float|list|dict|tuple|set|type|isinstance|hasattr|getattr|setattr|super|property|staticmethod|classmethod|abs|all|any|bin|bool|chr|dir|enumerate|eval|exec|filter|format|hex|id|input|map|max|min|next|oct|open|ord|pow|repr|reversed|round|sorted|sum|vars|zip)\b"
            ),
        ),
    ...
    Language.JAVASCRIPT: [
        (
            "keyword",
            re.compile(
                r"\b(async|await|break|case|catch|class|const|continue|debugger|default|delete|do|else|export|extends|finally|for|from|function|if|import|in|instanceof|let|new|of|return|switch|this|throw|try|typeof|var|void|while|with|yield|true|false|null|undefined)\b"
            ),
        ),
    ...
    Language.BASH: [
        (
            "keyword",
            re.compile(
                r"\b(if|then|else|elif|fi|for|while|do|done|case|esac|in|function|select|until|time|coproc|return|exit|break|continue|declare|export|local|readonly|typeset|unset|source|alias|echo|cd|pwd|ls|mkdir|rm|cp|mv|cat|grep|sed|awk|find|sort|uniq|wc|head|tail|chmod|chown|sudo|apt|yum|pip|npm|git|docker)\b"
            ),
        ),
        ("string", re.compile(r"(\"[^\"]*\"|\'[^\']*\'|`[^`]*`)")),
        ("comment", re.compile(r"(#[^\n]*)")),
    ...
    Language.RUST: [
        (
            "keyword",
            re.compile(
                r"\b(as|async|await|break|const|continue|crate|dyn|else|enum|extern|fn|for|if|impl|in|let|loop|match|mod|move|mut|pub|ref|return|self|Self|static|struct|super|trait|type|unsafe|use|where|while|yield|true|false|Some|None|Ok|Err)\b"
            ),
        ),
    ...
    Language.GO: [
        (
            "keyword",
            re.compile(
                r"\b(break|case|chan|const|continue|default|defer|else|fallthrough|for|func|go|goto|if|import|interface|map|package|range|return|select|struct|switch|type|var|true|false|nil|iota|append|cap|close|copy|delete|len|make|new|panic|print|println|recover)\b"
            ),
        ),
        ("string", re.compile(r"(\"[^\"]*\"|`[\s\S]*?`)")),
        ("comment", re.compile(r"(//[^\n]*|/\*[\s\S]*?\*/)")),
