        ...
        for i in range(self._height):
            ...
            if row < len(self._lines):
                ...
                if row == self._cursor_row and i < self._height:
                    ...
                    content = (
                        ansi.color(display_text, self._style.text_style) if display_text else ""
                    )
...
class SelectBox:
    ...
    def render(self, visible_height: int = 10) -> str:
        ...
        if len(self._options) > visible_height:
            lines.append(
                ansi.color(
                    f"  (showing {self._scroll_offset + 1}-{visible_end} of {len(self._options)})",
                    "dim",
                )
            )
...
class ConfirmDialog:
    ...
    def render(self) -> str:
        ...
        options = "[Y/n]" if self._default else "[y/N]"
...
class InputBox:
    ...
    def render(self) -> str:
        ...
        # Prompt
        prompt_line = (
            f"{ansi.color(self._style.prompt_char, self._style.highlight_style)} {self._prompt}"
        )
        if self._default:
        ...
        if self._error:
            lines.append(
                ansi.color(f"  {self._style.cross_char} {self._error}", self._style.error_style)
            )
