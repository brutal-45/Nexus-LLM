...
class CommandHandler:
    ...
    def _register_all(self) -> None:
        ...
        self._register(
            "/model", "Switch to a different model", self._cmd_model, usage="/model <name>"
        )
        ...
        self._register(
            "/config", "View or set configuration", self._cmd_config, usage="/config [key] [value]"
        )
        ...
        self._register(
            ...
            self._cmd_load_chat,
            usage="/load-chat <path>",
        )
        self._register(
            "/export", "Export conversation as Markdown", self._cmd_export, usage="/export [path]"
        )
        ...
        self._register(
            "/stream", "Toggle streaming (on/off)", self._cmd_stream, usage="/stream [on|off]"
        )
        ...
        self._register(
            "/maxlen", "Set max generation length", self._cmd_maxlen, usage="/maxlen <tokens>"
        )
        ...
        self._register(
            "/train", "Start training on a dataset", self._cmd_train, usage="/train <dataset>"
        )
        self._register(
            "/server",
        ...
        self._register(
            "/download", "Download a model", self._cmd_download, usage="/download <model>"
        )
    ...
    def _cmd_help(self, args: str, ctx: Dict[str, Any]) -> None:
        ...
        table = Table(
            title="Commands", show_header=True, border_style=self.formatter.theme.border_color
        )
    ...
    def _cmd_history(self, args: str, ctx: Dict[str, Any]) -> None:
        ...
        table = Table(
            title="Chat History", show_header=True, border_style=self.formatter.theme.border_color
        )
    ...
    def _cmd_themes(self, args: str, ctx: Dict[str, Any]) -> None:
        ...
        themes = list_themes()
        table = Table(
            title="Themes", show_header=True, border_style=self.formatter.theme.border_color
        )
    ...
    def _cmd_stream(self, args: str, ctx: Dict[str, Any]) -> None:
        ...
        if not args:
            self.formatter.print_info(
                f"Streaming: {'on' if settings.terminal.streaming else 'off'}"
            )
            return
    ...
    def _cmd_server(self, args: str, ctx: Dict[str, Any]) -> None:
        ...
        if action == "start":
            ...
            try:
                ...
                self.formatter.print_info(
                    "Server would start here (blocking call omitted in terminal mode)."
                )
            ...
            self.formatter.print_info(
                "Server stop requested (no server running from this terminal)."
            )
        else:
    ...
    def _cmd_download(self, args: str, ctx: Dict[str, Any]) -> None:
        ...
        if model_id not in MODEL_CATALOG:
            self.formatter.print_error(
                f"Model '{model_id}' not found in catalog. Use /models to see available models."
            )
            return
        ...
        self.formatter.print_info(
            "This will cache the model on first load. Use /load to load it after download."
        )
