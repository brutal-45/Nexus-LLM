...
COMMAND_OPTIONS: dict[str, list[dict[str, str]]] = {
    "chat": [
        ...
        {
            "flags": "--preset -p",
            "desc": "Chat preset (creative|precise|balanced|coding|writing|research)",
        },
...
class CompletionGenerator:
    ...
    def __init__(
        ...
        global_options: list[dict[str, str]] | None = None,
        command_options: dict[str, list[dict[str, str]]] | None = None,
    ...
    def generate_bash(self) -> str:
        ...
        for cmd, opts in self.command_options.items():
            opt_str = " ".join(
                flag
                for opt in opts
                for flag in opt.get("flags", "").split()
                if flag.startswith("-")
            )
        ...
        lines.extend(
            [
                ...
                "  if [[ $cur == -* ]]; then",
                '    local cmd="${words[1]}"',
                '    local cmd_opts_var="${cmd}_opts"',
                '    local cmd_opts="${!cmd_opts_var}"',
                '    COMPREPLY=($(compgen -W "${global_opts} ${cmd_opts}" -- "$cur"))',
                "    return",
                "  fi",
                "",
    ...
    def generate_fish(self) -> str:
        ...
        for choice_key, mappings in choice_option_map.items():
            ...
            for cmd, opt in mappings:
                cmd_cond = f'-n "__fish_seen_subcommand_from {cmd}"' if cmd else ""
                opt_flag = (
                    f'-l {opt.lstrip("--")}' if opt.startswith("--") else f'-s {opt.lstrip("-")}'
                )
                choices_str = " ".join(choices)
                lines.append(
                    f"complete -c {self.program_name} {cmd_cond} " f'{opt_flag} -a "{choices_str}"'
                )
            lines.append("")
