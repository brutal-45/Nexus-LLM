...
class JSONTool(BaseTool):
    ...
    def parameters(self) -> List[ToolParameter]:
        return [
            ...
            ToolParameter(
                name="path",
                type=ParameterType.STRING,
                description="Dot-notation path (for get/set/delete)",
                required=False,
            ),
            ToolParameter(
                name="value",
                type=ParameterType.STRING,
                description="Value to set (JSON encoded)",
                required=False,
            ),
            ToolParameter(
                name="data2",
                type=ParameterType.STRING,
                description="Second JSON string (for merge/diff)",
                required=False,
            ),
        ]
    ...
    def _get(self, data: str, path: str = "", **kw: Any) -> ToolResult:
        if not path:
            return ToolResult(
                tool_name=self.name, success=False, error="Path is required for 'get'"
            )
        obj = json.loads(data)
    ...
    def _set(self, data: str, path: str = "", value: str = "", **kw: Any) -> ToolResult:
        if not path:
            return ToolResult(
                tool_name=self.name, success=False, error="Path is required for 'set'"
            )
        obj = json.loads(data)
    ...
    def _delete(self, data: str, path: str = "", **kw: Any) -> ToolResult:
        if not path:
            return ToolResult(
                tool_name=self.name, success=False, error="Path is required for 'delete'"
            )
        obj = json.loads(data)
        self._delete_path(obj, path)
        return ToolResult(
            tool_name=self.name, success=True, output=obj, metadata={"deleted_path": path}
        )
    ...
    def _merge(self, data: str, data2: str = "", **kw: Any) -> ToolResult:
        if not data2:
            return ToolResult(
                tool_name=self.name, success=False, error="data2 is required for 'merge'"
            )
    ...
    def _diff(self, data: str, data2: str = "", **kw: Any) -> ToolResult:
        if not data2:
            return ToolResult(
                tool_name=self.name, success=False, error="data2 is required for 'diff'"
            )
        obj1 = json.loads(data)
        obj2 = json.loads(data2)
            ...
            for key in all_keys:
                ...
                if key not in a:
                    ...
                    if isinstance(a[key], dict) and isinstance(b[key], dict):
                        ...
                        diffs.append(
                            {"path": path, "change": "modified", "old": a[key], "new": b[key]}
                        )
        return diffs
