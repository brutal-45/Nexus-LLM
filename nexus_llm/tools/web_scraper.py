...
class WebScraperTool(BaseTool):
    ...
    def parameters(self) -> List[ToolParameter]:
        return [
            ToolParameter(
                name="url", type=ParameterType.STRING, description="URL to fetch", required=True
            ),
            ToolParameter(
                name="mode",
    ...
    def execute(
        self, url: str = "", mode: str = "text", timeout: int = 30, **kwargs: Any
    ) -> ToolResult:
        ...
        try:
            ...
            return ToolResult(
                tool_name=self.name, success=False, error=f"HTTP {exc.code}: {exc.reason}"
            )
        ...
        if mode == "raw":
            return ToolResult(
                tool_name=self.name, success=True, output=html, metadata={"url": url, "mode": mode}
            )
        ...
        if mode == "links":
            ...
            return ToolResult(
                ...
                success=True,
                output=links,
                metadata={"url": url, "link_count": len(links)},
            )
