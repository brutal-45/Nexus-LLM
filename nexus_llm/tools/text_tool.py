...
from nexus_llm.tools.base_tool import BaseTool, ParameterType, ToolParameter, ToolResult
...
class TextTool(BaseTool):
    ...
    def parameters(self) -> List[ToolParameter]:
        return [
            ...
            ToolParameter(
                name="text", type=ParameterType.STRING, description="Input text", required=True
            ),
            ToolParameter(
                name="pattern",
                type=ParameterType.STRING,
                description="Search pattern or delimiter",
                required=False,
            ),
            ToolParameter(
                name="replacement",
            ...
            ToolParameter(
                ...
                description="Max length for truncate/wrap",
                required=False,
                default=80,
            ),
        ]
    ...
    def execute(
        ...
        **kwargs: Any,
        ...
        if handler is None:
            return ToolResult(
                tool_name=self.name, success=False, error=f"Unknown operation: {operation}"
            )
    ...
    def _search(self, text: str, pattern: str = "", **kw: Any) -> ToolResult:
        if not pattern:
            return ToolResult(
                tool_name=self.name, success=False, error="Pattern required for search"
            )
    ...
    def _replace(
        self, text: str, pattern: str = "", replacement: str = "", **kw: Any
    ) -> ToolResult:
        if not pattern:
            return ToolResult(
                tool_name=self.name, success=False, error="Pattern required for replace"
            )
        result = re.sub(pattern, replacement, text)
        changes = len(re.findall(pattern, text))
        return ToolResult(
            tool_name=self.name, success=True, output=result, metadata={"replacements": changes}
        )
    ...
    def _split(self, text: str, pattern: str = "", **kw: Any) -> ToolResult:
        ...
        return ToolResult(
            tool_name=self.name, success=True, output=parts, metadata={"part_count": len(parts)}
        )
    ...
    def _count(self, text: str, pattern: str = "", **kw: Any) -> ToolResult:
        ...
        return ToolResult(
            ...
            success=True,
            output=count,
            metadata={"pattern": pattern or "chars"},
        )
    ...
    def _pad(self, text: str, max_length: int = 80, **kw: Any) -> ToolResult:
        ...
        return ToolResult(
            tool_name=self.name, success=True, output=padded, metadata={"width": max_length}
        )
    ...
    def _wrap(self, text: str, max_length: int = 80, **kw: Any) -> ToolResult:
        ...
        return ToolResult(
            tool_name=self.name, success=True, output=wrapped, metadata={"width": max_length}
        )
