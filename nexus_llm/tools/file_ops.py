...
class FileOpsTool(BaseTool):
    ...
    def __init__(self, sandbox_root: str = _DEFAULT_SANDBOX) -> None:
        super().__init__(
            name="file_ops", description="Safe file system operations within a sandbox"
        )
    ...
    def _append(self, path: str, content: str) -> ToolResult:
        ...
        return ToolResult(
            tool_name=self.name,
            success=True,
            output=f"Appended {len(content)} chars",
            metadata={"path": path},
        )
    ...
    def _delete(self, path: str) -> ToolResult:
        if os.path.isfile(path):
            ...
            shutil.rmtree(path)
            return ToolResult(
                tool_name=self.name, success=True, output=f"Deleted directory: {path}"
            )
