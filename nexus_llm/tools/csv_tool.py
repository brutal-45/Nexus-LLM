...
class CsvTool(BaseTool):
    ...
    def parameters(self) -> List[ToolParameter]:
        return [
            ToolParameter(
                ...
                required=True,
                choices=["read", "write", "filter", "transform", "headers"],
            ),
            ToolParameter(
                name="data", type=ParameterType.STRING, description="CSV data string", required=True
            ),
            ToolParameter(
                name="delimiter",
                type=ParameterType.STRING,
    ...
    def _read(self, data: str, delimiter: str, has_header: bool) -> ToolResult:
        ...
        return ToolResult(
            success=True,
            output=result,
            metadata={"row_count": len(result) if has_header else len(rows)},
        )
    ...
    def _filter(
        self, data: str, delimiter: str, has_header: bool, column: str, value: str
    ) -> ToolResult:
