...
class MathTool(BaseTool):
    ...
    def parameters(self) -> List[ToolParameter]:
        return [
            ...
            ToolParameter(
                ...
                description="Single integer parameter",
                required=False,
            ),
            ToolParameter(
                name="k",
                type=ParameterType.INTEGER,
                description="Second integer parameter (for nCk, nPk)",
                required=False,
            ),
        ]

    def execute(
        self, operation: str = "", numbers: str = "", n: int = 0, k: int = 0, **kwargs: Any
    ) -> ToolResult:
        ...
        try:
            ...
            if operation == "stats":
                ...
                return ToolResult(
                    tool_name=self.name, success=True, output=math.factorial(n), metadata={"n": n}
                )
            elif operation == "fibonacci":
                return self._fibonacci(n)
            elif operation == "is_prime":
                return ToolResult(
                    tool_name=self.name, success=True, output=self._is_prime(n), metadata={"n": n}
                )
            elif operation == "prime_factors":
                return ToolResult(
                    tool_name=self.name,
                    success=True,
                    output=self._prime_factors(n),
                    metadata={"n": n},
                )
    ...
    def _gcd_op(self, nums: List[float]) -> ToolResult:
        if len(nums) < 2:
            return ToolResult(
                tool_name=self.name, success=False, error="At least 2 numbers required"
            )
        result = int(nums[0])
        for num in nums[1:]:
    ...
    def _distance(self, nums: List[float]) -> ToolResult:
        if len(nums) != 4:
            return ToolResult(
                tool_name=self.name, success=False, error="Need 4 numbers: x1,y1,x2,y2"
            )
        x1, y1, x2, y2 = nums
