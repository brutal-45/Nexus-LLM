...
class DateTimeTool(BaseTool):
    ...
    def parameters(self) -> List[ToolParameter]:
        return [
            ...
            ToolParameter(
                name="datetime_str",
                type=ParameterType.STRING,
                description="Datetime string to operate on",
                required=False,
            ),
            ToolParameter(
                name="format",
                type=ParameterType.STRING,
                description=" strftime/strptime format string",
                required=False,
                default="%Y-%m-%d %H:%M:%S",
            ),
            ToolParameter(
            ...
            ToolParameter(
                name="datetime_str2",
                type=ParameterType.STRING,
                description="Second datetime for diff",
                required=False,
            ),
        ]
    ...
    def _parse(self, datetime_str: str, fmt: str) -> ToolResult:
        ...
        return ToolResult(
            tool_name=self.name, success=True, output=dt.isoformat(), metadata={"parsed": True}
        )

    def _add(
        self, datetime_str: str, fmt: str, days: int, hours: int, minutes: int, seconds: int
    ) -> ToolResult:
