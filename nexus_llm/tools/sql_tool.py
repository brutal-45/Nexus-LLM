...
class SQLTool(BaseTool):
    ...
    def __init__(self) -> None:
        super().__init__(
            name="sql", description="Execute SQL queries against an in-memory SQLite database"
        )
    ...
    def parameters(self) -> List[ToolParameter]:
        return [
            ...
            ToolParameter(
                name="table_name",
                type=ParameterType.STRING,
                description="Table name for schema op",
                required=False,
            ),
            ToolParameter(
                name="csv_data",
                type=ParameterType.STRING,
                description="CSV data for load_csv",
                required=False,
            ),
        ]
    ...
    def _schema(self, table_name: str) -> ToolResult:
        ...
        return ToolResult(
            tool_name=self.name, success=True, output=columns, metadata={"table": table_name}
        )
    ...
    def _load_csv(self, table_name: str, csv_data: str) -> ToolResult:
        ...
        if not table_name or not csv_data:
            return ToolResult(
                tool_name=self.name, success=False, error="table_name and csv_data required"
            )
        ...
        for row in reader:
            ...
            self._conn.execute(f'INSERT INTO "{table_name}" VALUES ({placeholders})', values)
            rows_inserted += 1
