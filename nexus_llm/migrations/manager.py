...
class MigrationManager:
    ...
    def register(self, migration: Migration) -> None:
        ...
        if migration.version in self._migrations:
            raise ValueError(f"Migration version {migration.version!r} is already registered")
    ...
    def rollback(self, steps: int = 1) -> List[str]:
        ...
        applied_versions = [
            v for v in reversed(self._ordered_versions) if self._history.is_applied(v)
        ]
        ...
        for version in applied_versions[:steps]:
            ...
            try:
                ...
                raise RuntimeError(f"Rollback of migration {version} failed: {exc}") from exc
        return rolled_back
    ...
    def get_pending(self) -> List[str]:
        ...
        """
        return [v for v in self._ordered_versions if not self._history.is_applied(v)]
