...
def _generate_arc_example(rng: random.Random, difficulty: str) -> dict[str, Any]:
    ...
    questions = {
        ...
        "hard": [
            ...
            {
                "question": "In quantum mechanics, what principle states that certain pairs of properties cannot both be precisely known?",
                "choices": [
                    "Pauli exclusion",
                    "Superposition",
            ...
            },
            {
                ...
                "choices": ["Polymerase", "Ligase", "Helicase", "Primase"],
                "answer": 2,
            },
        ],
    }
...
def _generate_winogrande_example(rng: random.Random) -> dict[str, Any]:
    ...
    examples = [
        {
            "sentence": "The trophy didn't fit into the brown suitcase because ___ was too small.",
            "choices": ["the trophy", "the suitcase"],
            "answer": 1,
        },
        {
            "sentence": "The city councilmen refused the demonstrators a permit because ___ feared violence.",
            "choices": ["the councilmen", "the demonstrators"],
            "answer": 0,
        },
        {
            "sentence": "Joan made sure to thank Susan for all the help she had ___.",
            "choices": ["given", "received"],
            "answer": 1,
        },
        {
            "sentence": "The police arrested the protesters because ___ threatened public safety.",
            "choices": ["the police", "the protesters"],
            "answer": 1,
        },
        {
            "sentence": "Mary gave Joan a beautiful bouquet because ___ was graduating.",
            "choices": ["Mary", "Joan"],
            "answer": 1,
        },
    ]
...
class BenchmarkRunner:
    ...
    def __init__(
        ...
        category_accuracy_offset: dict[str, float] | None = None,
        seed: int = 42,
    ):
    ...
    def _simulate_prediction(
        self, rng: random.Random, answer: int, num_choices: int, accuracy: float
    ) -> int:
    ...
    def run_mmlu(self, config: BenchmarkConfig | None = None) -> BenchmarkResult:
        """Run simulated MMLU benchmark."""
        if config is None:
            config = BenchmarkConfig(benchmark_type=BenchmarkType.MMLU)
        rng = random.Random(config.seed)
        categories = config.categories or rng.sample(
            _MMLU_CATEGORIES, min(10, len(_MMLU_CATEGORIES))
        )
    ...
    def run_hellaswag(self, config: BenchmarkConfig | None = None) -> BenchmarkResult:
    ...
    def run_arc(self, config: BenchmarkConfig | None = None) -> BenchmarkResult:
    ...
    def run_winogrande(self, config: BenchmarkConfig | None = None) -> BenchmarkResult:
