...
from nexus_llm.workflow.engine import WorkflowConfig, WorkflowEngine
from nexus_llm.workflow.nodes import NodeType, WorkflowNode
...
class SequentialTemplate(WorkflowTemplate):
    ...
    def build(self, steps: Optional[List[Callable]] = None, **kwargs: Any) -> WorkflowEngine:
        ...
        for i, step_fn in enumerate(steps):
            step_id = f"step_{i}"
            engine.add_node(WorkflowNode(id=step_id, type=NodeType.TASK, fn=step_fn))
...
class FanOutFanInTemplate(WorkflowTemplate):
    ...
    def build(
        self,
        branches: Optional[List[Callable]] = None,
        ...
        **kwargs: Any,
        ...
        for i, branch_fn in enumerate(branches):
            ...
            engine.add_node(WorkflowNode(id=branch_id, type=NodeType.TASK, fn=branch_fn))
...
class ConditionalBranchTemplate(WorkflowTemplate):
    ...
    def build(
        ...
        **kwargs: Any,
        ...
        engine.add_node(
            WorkflowNode(id="true_path", type=NodeType.TASK, fn=true_fn or (lambda x: x))
        )
        engine.add_node(
            WorkflowNode(id="false_path", type=NodeType.TASK, fn=false_fn or (lambda x: x))
        )
        engine.add_node(WorkflowNode(id="end", type=NodeType.END, fn=lambda x: x))
...
class RetryTemplate(WorkflowTemplate):
    ...
    def build(
        ...
        **kwargs: Any,
        ...
        engine.add_node(WorkflowNode(id="start", type=NodeType.START, fn=lambda: None))
        engine.add_node(WorkflowNode(id="process", type=NodeType.TASK, fn=retry_wrapper))
        engine.add_node(WorkflowNode(id="end", type=NodeType.END, fn=lambda x: x))
