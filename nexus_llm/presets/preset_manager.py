...
class PresetNotFoundError(PresetError):
    ...
    def __init__(self, preset_name: str, category: str) -> None:
        ...
        super().__init__(f"Preset '{preset_name}' not found in category '{category}'")
...
class PresetValidationError(PresetError):
    ...
    def __init__(self, preset_name: str, errors: List[str]) -> None:
        self.preset_name = preset_name
        self.errors = errors
        super().__init__(f"Preset '{preset_name}' validation failed: {'; '.join(errors)}")
...
class PresetManager:
    ...
    def register_category(self, category: str, filepath: Union[str, Path]) -> None:
    ...
    def _load_category(self, category: str) -> Dict[str, Any]:
        ...
        if not filepath.exists():
            raise PresetNotFoundError(category, f"__file__ ({filepath})")
        ...
        *,
        ...
        for key, value in override.items():
            if key in result and isinstance(result[key], dict) and isinstance(value, dict):
                result[key] = PresetManager._deep_merge(result[key], value, overwrite=overwrite)
            elif key not in result or overwrite:
                result[key] = copy.deepcopy(value)
        return result

    @staticmethod
    def _validate_chat_preset(name: str, preset: Dict[str, Any]) -> List[str]:
        ...
        for field in required:
            if field not in gen:
                errors.append(f"Chat preset '{name}' missing generation field '{field}'")
        temp = gen.get("temperature")
        if temp is not None and not (0.0 <= temp <= 2.0):
            errors.append(f"Chat preset '{name}' temperature {temp} out of range [0, 2]")
        return errors

    @staticmethod
    def _validate_training_preset(name: str, preset: Dict[str, Any]) -> List[str]:
        ...
        for field in required:
            if field not in train:
                errors.append(f"Training preset '{name}' missing training field '{field}'")
        lr = train.get("learning_rate")
        if lr is not None and lr <= 0:
            errors.append(f"Training preset '{name}' has invalid learning_rate: {lr}")
        return errors

    @staticmethod
    def _validate_server_preset(name: str, preset: Dict[str, Any]) -> List[str]:
        ...
        for field in required:
            if field not in server:
                errors.append(f"Server preset '{name}' missing server field '{field}'")
        port = server.get("port")
        if port is not None and not (1 <= port <= 65535):
            errors.append(f"Server preset '{name}' port {port} out of valid range")
        return errors
