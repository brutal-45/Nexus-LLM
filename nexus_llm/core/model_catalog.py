...
MODEL_CATALOG: dict[str, ModelInfo] = {
    ...
    "smollm-1.7b": ModelInfo(
        id="smollm-1.7b",
        name="SmolLM 1.7B",
        hf_id="HuggingFaceTB/SmolLM-1.7B",
        category="smollm",
        size="1.7B",
        params="1.7B",
...
def get_model_info(model_id: str) -> ModelInfo:
    ...
    if model_id not in MODEL_CATALOG:
        ...
        raise ModelNotFoundError(f"Model '{model_id}' not found. Available models: {available}")
...
def list_categories() -> list[str]:
    ...
    return sorted({m.category for m in MODEL_CATALOG.values()})
