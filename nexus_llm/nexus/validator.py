...
class ValidationResult:
    ...
    def add_issue(
        self, level: ValidationLevel, message: str, field: str = "", value: Any = None
    ) -> None:
...
class ResponseValidator:
    ...
    def _min_length_rule(self, response: str, min_length: int = 1) -> List[ValidationIssue]:
        if len(response) < min_length:
            return [
                ValidationIssue(
                    ValidationLevel.ERROR, f"Response too short ({len(response)} < {min_length})"
                )
            ]
        return []
    ...
    def _max_length_rule(self, response: str, max_length: int = 10000) -> List[ValidationIssue]:
        if len(response) > max_length:
            return [
                ValidationIssue(
                    ValidationLevel.WARNING, f"Response too long ({len(response)} > {max_length})"
                )
            ]
        return []
    ...
    def _regex_rule(self, response: str, pattern: str = "") -> List[ValidationIssue]:
        if pattern and not re.search(pattern, response):
            return [
                ValidationIssue(
                    ValidationLevel.ERROR, f"Response does not match pattern: {pattern}"
                )
            ]
        return []
    ...
    def _profanity_rule(
        self, response: str, words: Optional[List[str]] = None
    ) -> List[ValidationIssue]:
        bad_words = words or []
        found = [w for w in bad_words if w.lower() in response.lower()]
        if found:
            return [
                ValidationIssue(ValidationLevel.WARNING, "Potentially inappropriate content found")
            ]
        return []
