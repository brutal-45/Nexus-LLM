...
class Engine:
    ...
    def start(self) -> None:
        ...
        if self._state == EngineState.ERROR:
            raise NexusLLMError(
                "Engine is in error state; call reset() first", error_code="ENGINE_ERROR_STATE"
            )
        ...
        try:
            ...
            raise InferenceError(
                message=f"Engine startup failed: {exc}", error_code="ENGINE_STARTUP_FAILED"
            ) from exc
    ...
    def submit(self, job_fn: Callable, job_id: Optional[str] = None, **kwargs: Any) -> JobResult:
        ...
        if self._state not in (EngineState.READY, EngineState.RUNNING):
            raise InferenceError(
                message="Engine is not ready for jobs", error_code="ENGINE_NOT_READY"
            )
        ...
        try:
            ...
            duration_ms = (time.perf_counter() - start) * 1000
            result = JobResult(
                job_id=job_id, success=False, error=str(exc), duration_ms=duration_ms
            )
