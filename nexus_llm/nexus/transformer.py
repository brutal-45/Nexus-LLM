...
class Transformer:
    ...
    def apply(self, text: str, transform_name: str) -> TransformResult:
        ...
        if transform_name not in self._transforms:
            raise ValueError(
                f"Unknown transform: {transform_name}. Available: {list(self._transforms.keys())}"
            )
        ...
        return TransformResult(
            ...
            changes_count=sum(1 for a, b in zip(text, transformed) if a != b)
            + abs(len(text) - len(transformed)),
        )
    ...
    def apply_custom(
        self, text: str, fn: Callable[[str], str], name: str = "custom"
    ) -> TransformResult:
