...
_BUILTIN_TEMPLATES: Dict[str, Dict[str, Any]] = {
    ...
    "review": {
        ...
        "template_type": TemplateType.INSTRUCTION,
        "system_prompt": (
            "You are a thorough reviewer. Provide constructive, " "detailed feedback."
        ),
...
class PromptLibrary:
    ...
    def get_by_category(self, category: Optional[str] = None) -> Dict[str, PromptTemplate]:
        ...
        try:
            ...
            raise ValueError(
                f"Invalid category {category!r}. " f"Valid: {[t.value for t in TemplateType]}"
            )

        return {
            name: tpl for name, tpl in self._templates.items() if tpl.template_type == target_type
        }
