...
class Evaluator:
    ...
    def evaluate_generation_quality(
        ...
        samples = self.generate_samples(prompts, tokenizer, max_new_tokens=max_new_tokens)
        ...
        if references and len(references) == len(samples):
            rouge_scores = self._compute_rouge_approx([s["generated"] for s in samples], references)
    ...
    def _compute_rouge_approx(
        ...
        for pred, ref in zip(predictions, references):
            ...
            pred_bigrams = set(zip(pred.lower().split(), pred.lower().split()[1:]))
            ref_bigrams = set(zip(ref.lower().split(), ref.lower().split()[1:]))
            if ref_bigrams:
    ...
    def full_evaluation(
        ...
        if tokenizer and sample_prompts:
            gen_metrics = self.evaluate_generation_quality(sample_prompts, tokenizer=tokenizer)
