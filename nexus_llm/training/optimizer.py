...
class OptimizerConfig:
    ...
    no_decay_params: List[str] = field(
        default_factory=lambda: ["bias", "LayerNorm.weight", "layer_norm.weight"]
    )
...
def _build_adafactor(model: nn.Module, config: OptimizerConfig) -> torch.optim.Optimizer:
    ...
    try:
        ...
        param_groups = _get_param_groups(
            model,
            config.weight_decay,
            config.no_decay_params,
            config.separate_decay_groups,
            config.learning_rate,
        )
        return _build_adamw(param_groups, config)


def _build_8bit_adam(
    param_groups: List[Dict[str, Any]], config: OptimizerConfig
) -> torch.optim.Optimizer:
    ...
    try:
        ...
        logger.warning("bitsandbytes not available for 8-bit Adam. Falling back to standard AdamW.")
        return _build_adamw(param_groups, config)
