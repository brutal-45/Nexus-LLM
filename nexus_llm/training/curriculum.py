...
class PacingFunction:
    ...
    def _root_pacing(self, progress: float) -> float:
        return self.initial_fraction + (self.final_fraction - self.initial_fraction) * math.sqrt(
            progress
        )
    ...
    def _step_pacing(self, progress: float) -> float:
        ...
        return (
            ...
            + (self.final_fraction - self.initial_fraction) * (stage + 1) / num_stages
        )
    ...
    def _sigmoid_pacing(self, progress: float) -> float:
        ...
        return (
            self.initial_fraction
            + (self.final_fraction - self.initial_fraction) * sigmoid_normalized
        )
...
class CurriculumLearner:
    ...
    def get_staged_data(self, num_stages: Optional[int] = None) -> List[List[Dict[str, Any]]]:
        ...
        logger.info(
            f"Created {len(stages)} curriculum stages with sizes: {[len(s) for s in stages]}"
        )
        return stages
