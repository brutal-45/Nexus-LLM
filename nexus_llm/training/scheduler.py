...
def _get_cosine_with_restarts_lambda(warmup_steps: int, total_steps: int, num_cycles: float = 1.0):
...
def get_warmup_decay_per_param_group(
    ...
    **kwargs,
    ...
    if scheduler_type not in schedulers:
        raise ValueError(
            f"Unknown scheduler type: {scheduler_type}. " f"Choose from {list(schedulers.keys())}"
        )
