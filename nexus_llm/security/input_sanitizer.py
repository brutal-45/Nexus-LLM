...
class InputSanitizer:
    ...
    def __init__(self, max_length: int = 10000, strict_mode: bool = False) -> None:
        ...
        logger.debug(
            "InputSanitizer initialized (max_length=%d, strict=%s)", max_length, strict_mode
        )
    ...
    def sanitize(self, text: str) -> SanitizationResult:
        ...
        cleaned = "".join(ch for ch in current if ch.isprintable() or ch in "\n\r\t")
        ...
        result.changes_made = sum(1 for a, b in zip(text, cleaned) if a != b) + abs(
            len(text) - len(cleaned)
        )

        if not result.is_safe:
            logger.warning(
                "Input sanitization found %d issues: %s", len(result.issues), result.issues[:3]
            )
    ...
    def _sanitize_html(self, text: str) -> tuple:
        ...
        dangerous_tags = re.findall(
            r"<\s*(script|iframe|object|embed|form|input)[^>]*>", text, re.IGNORECASE
        )
        if dangerous_tags:
