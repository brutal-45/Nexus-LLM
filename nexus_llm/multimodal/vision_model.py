...
class MultimodalInput:
    ...
    def from_image_paths(
        cls, paths: List[str], prompt: str = "Describe these images."
    ) -> MultimodalInput:
...
class VisionLanguageModel:
    ...
    def load_model(self) -> None:
        ...
        try:
            ...
            raise ImportError(
                "VLM loading requires transformers and torch: " "pip install transformers torch"
            )
        ...
        self._model = AutoModelForVision2Seq.from_pretrained(self._config.model_name, **kwargs)
    ...
    def get_info(self) -> Dict[str, Any]:
        ...
        if self._loaded and self._model is not None:
            ...
            info["parameter_count_human"] = (
                f"{param_count / 1e9:.2f}B" if param_count >= 1e9 else f"{param_count / 1e6:.2f}M"
            )
        return info
