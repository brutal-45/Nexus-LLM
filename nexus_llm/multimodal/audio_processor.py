...
class AudioProcessor:
    ...
    def _load_mp3(self, path: str) -> AudioData:
        ...
        try:
            ...
            raise ImportError(
                "MP3 loading requires pydub and ffmpeg: " "pip install pydub && install ffmpeg"
            )
    ...
    def _compute_rms(self, audio: AudioData) -> float:
        ...
        if HAS_NUMPY and isinstance(audio.samples, np.ndarray):
            ...
            s = (
                np.array(list(audio.samples), dtype=float)
                if HAS_NUMPY
                else list(map(float, audio.samples))
            )
    ...
    def _compute_zcr(self, audio: AudioData) -> float:
        ...
        if HAS_NUMPY and isinstance(audio.samples, np.ndarray):
            ...
            s_list = list(map(float, audio.samples))
            crossings = sum(
                1 for i in range(1, len(s_list)) if (s_list[i] >= 0) != (s_list[i - 1] >= 0)
            )
    ...
    def split_channels(self, audio: AudioData) -> List[AudioData]:
        ...
        if HAS_NUMPY and isinstance(audio.samples, np.ndarray):
            ...
            for ch in range(audio.channels):
                ...
                channels.append(
                    AudioData(samples=ch_samples, sample_rate=audio.sample_rate, channels=1)
                )
            return channels
    ...
    def _decode_pcm(raw: bytes, sample_width: int, channels: int) -> Any:
        ...
        if HAS_NUMPY:
            ...
            for i in range(n):
                val = int.from_bytes(
                    raw[i * sample_width : (i + 1) * sample_width], "little", signed=True
                )
