            ...
            summary.update(
                {
                    ...
                    "peak_delta_mb": (
                        max(s.get("delta_mb", 0) for s in self._snapshots if "delta_mb" in s)
                        if any("delta_mb" in s for s in self._snapshots)
                        else 0
                    ),
                }
            )
...
class FunctionProfiler:
    ...
    def get_summary(self) -> str:
        ...
        lines = []
        lines.append(
            f"{'Function':<40} {'Calls':>8} {'Total(s)':>10} {'Avg(s)':>10} {'Min(s)':>10} {'Max(s)':>10}"
        )
        lines.append("-" * 88)
