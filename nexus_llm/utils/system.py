...
def print_system_info():
    ...
    logger.info(
        f"  CPU Cores: {cpu.get('count_logical', 'Unknown')} logical, {cpu.get('count_physical', 'Unknown')} physical"
    )

    mem = info.get("memory", {})
    logger.info(
        f"  RAM: {mem.get('total_gb', 'Unknown')} GB ({mem.get('percent_used', 'N/A')}% used)"
    )
