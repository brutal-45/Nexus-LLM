...
def validate_model_name(name: str) -> ModelInfo:
    ...
    raise ModelNotFoundError(f"Model '{name}' not found in catalog. Available models: {available}")
...
def download_model(
    ...
    try:
        ...
        raise ModelLoadError(f"Failed to download model '{hf_id}': {exc}") from exc
